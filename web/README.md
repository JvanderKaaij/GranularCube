...
Choose a PNG, JPEG, or WebP image in the **IMAGE / SCENE INPUT** area of `mood~`, optionally add direction in the mood text box, and press **IMAGE → PATCH + SFX**. The browser prepares at least three granular modules and one bell, adding any missing modules, then sends the image as a base64 data URL alongside a prompt containing the complete patch, built-in sample catalog, instrument capabilities, and JSON response example to `POST http://localhost:8000/api/image-patch`. The API uses `OPENAI_VISION_MODEL` (default `gpt-4o-mini`) to describe the visible scene and return a musical mood, a composition outline, master and module settings, a bell note sequence, and a role for each instrument. The generated mood replaces the text in **MOOD / INTENTION**. The prompt asks for one recognizable SFX tied to the image, while the other granular modules can turn existing samples into a pad, lead, or rhythmic/percussive layer. The bell plays a new note pattern shaped by the mood. The model shapes grain timing, filters, levels, notes, and all effects to connect the layers.

After validating the response, the browser sends the single selected keyword to `/api/sfx`, loads that generated sound into its assigned granular module, switches the other granular modules to their chosen built-in samples, and applies all settings over the selected transition time. Idle modules start at zero level and fade in during the transition. Continuous controls move over the whole transition; the bell root and note sequence switch at its midpoint. If the model repeats the exact current bell pattern, the patch varies its order so an image request still changes the notes. The scene description, composition outline, and resulting settings appear in `mood~`. If the request fails, automatically added modules are removed. This flow needs both `OPENAI_API_KEY` and `ELEVENLABS_API_KEY` on the server. Images are limited to 8 MB. The offline checks are `npm run test:patch` and `server/venv/bin/python server/test_image_patch.py` from the project root under WSL.
